    ...
    @php
        ...
        $selectedSheet = $sheetOptions->first(function ($sheet) use ($selectedRounds) {
            return in_array('overall', $selectedRounds, true)
                ...
                : in_array((string) $sheet->analysis_round_id, array_map('strval', $selectedRounds), true);
        });
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form method="GET" action="{{ route('analysis.show', $dataset) }}" class="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="lg:col-span-4 flex items-center justify-end gap-2 pt-4 border-t border-slate-100 mt-2">
                    ...
                    <a href="{{ route('analysis.show', $dataset) }}" class="rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-bold text-slate-600 transition hover:border-rose-300 hover:text-rose-600">
                        Reset
                    </a>
                </div>
            </form>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $showCourseColumn ? 3 : 2 }}" class="px-4 py-10 text-center text-sm font-semibold text-slate-500">
                                    No records found for these filters.
                                </td>
                            </tr>
                        @endforelse
