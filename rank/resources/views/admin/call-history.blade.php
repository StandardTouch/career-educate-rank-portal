...
<head>
    ...
    <title>Calls for {{ $user->name }} - Career Educate Admin</title>
...
<body class="min-h-screen bg-slate-50 text-slate-800">
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <section class="flex flex-col gap-4 border-b border-slate-200 pb-6 md:flex-row md:items-end md:justify-between">
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.18em] text-rose-500">Student Call History</p>
                <h1 class="mt-2 text-3xl font-extrabold text-slate-950">{{ $user->name }}</h1>
                <p class="mt-1 text-sm text-slate-500">{{ $user->phone ?? '-' }} {{ $user->email ? '| ' . $user->email : '' }}</p>
            </div>
        ...
        <section class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h2 class="text-base font-bold text-slate-900">Exotel Calls</h2>
                    <p class="mt-1 text-xs text-slate-400">Call records fetched using the student phone number.</p>
                </div>
                @if (!$apiError)
            ...
            @else
                ...
                <div class="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex items-center gap-2">
                        @if ($hasPreviousCalls)
                            <a href="{{ route('admin.call-history', ['user' => $user->id, 'call_page' => $currentCallPage - 1]) }}"
