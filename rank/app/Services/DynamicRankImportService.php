...
class DynamicRankImportService
{
    public function __construct(protected CourseDetectionService $courseDetection)
    {
    }

    public function importUploadedFile(UploadedFile $file, ?int $userId = null): Dataset
    {
        $originalName = $file->getClientOriginalName();
        $safeName = $this->safeFilename($originalName);
        $storedPath = $file->storeAs('rank-imports', now()->format('YmdHis') . '-' . $safeName);
    ...
    public function importFile(string $absolutePath, string $originalName, string $storedPath, ?int $userId = null, ?string $courseOverride = null): Dataset
    {
        ...
        try {
            ...
            Log::error('Dynamic rank import failed', [
                'dataset_id' => $dataset->id,
                'import_id' => $import->id,
                'file' => $originalName,
    ...
    protected function replaceDatasetRecords(string $absolutePath, Dataset $dataset, Import $import): int
    {
        ...
        RankRecord::where('dataset_id', $dataset->id)->delete();
        Round::where('dataset_id', $dataset->id)->delete();

        $totalRows = 0;

        if ($overallSheet !== null) {
            $importSheet = ImportSheet::create([
                'import_id' => $import->id,
    ...
    protected function importSheet($worksheet, Dataset $dataset, Import $import, ImportSheet $importSheet, ?Round $round): int
    {
        if ($worksheet === null) {
            return 0;
        }
        ...
        foreach ($rows as $row) {
            ...
            $record = [
                'dataset_id' => $dataset->id,
                'import_id' => $import->id,
                'import_sheet_id' => $importSheet->id,
                'round_id' => $round?->id,
    ...
    protected function createDatasetRound(Dataset $dataset, string $sheetName): Round
    {
    ...
    protected function metadataFromFilename(string $filename, ?string $courseOverride = null, array $extraCourseText = []): array
    {
        ...
        $slug = Str::slug(implode(' ', array_filter([$state, $course, $quota, $year, $descriptor])));
    ...
    protected function cleanMarks($value): ?float
    {
        ...
        // NEET marks are scores, not ranks or fees. Keep invalid source values in raw_payload,
        // but do not insert them into the bounded marks column.
    ...
    protected function safeFilename(string $filename): string
    {
        ...
        return ($name !== '' ? $name : 'rank-import') . '.' . strtolower($extension);
    }
}
