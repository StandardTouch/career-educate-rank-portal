...
<head>
    ...
    <title>Register - Career Educate</title>
    ...
    <main class="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <section class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-200">
                <p class="text-xs font-bold uppercase tracking-[0.18em] text-rose-500">Student Portal</p>
                <h1 class="mt-2 text-3xl font-bold text-slate-950">Create your account</h1>
                <p class="mt-1 text-xs text-slate-500">Register directly to access rank predictors and packages.</p>
            </div>

            <form action="{{ route('register.store') }}" method="POST" class="p-6 space-y-5">
                @csrf
                ...
                <div>
                    <label for="name" class="block text-sm font-bold uppercase tracking-wide text-slate-600">Full Name</label>
                ...
                <div>
                    <label for="email" class="block text-sm font-bold uppercase tracking-wide text-slate-600">Email Address</label>
                ...
                <button type="submit" class="w-full rounded-xl bg-rose-500 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-rose-500/10 transition hover:bg-rose-600 active:scale-95">
                    Register and Select Plan
                </button>

                <p class="text-center text-sm text-slate-500">
                    Already registered?
                    <a href="{{ route('login') }}" class="font-bold text-rose-500 hover:text-rose-600">Login</a>
                </p>
            </form>
        </section>
    </main>
