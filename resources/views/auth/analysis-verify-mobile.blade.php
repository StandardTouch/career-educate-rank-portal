...
<head>
    ...
    <title>Analysis Login - Career Educate</title>
    ...
    <main class="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <section class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-200">
                <p class="text-xs font-bold uppercase tracking-[0.18em] text-rose-500">Analysis Login</p>
                <h1 class="mt-2 text-3xl font-bold text-slate-950">Verify mobile number</h1>
            </div>

            <form action="{{ route('analysis.send-otp') }}" method="POST" class="p-6 space-y-5">
                @csrf
